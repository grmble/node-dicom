 ...
 */
ParseBuffer.prototype.requestStream = stopAwareFn("requestStream", function (bytes, callback) {
    this.requestQueue.push({bytes: bytes, callback: callback, stream: true});
    this.consumeRequests();
});
...
ParseBuffer.prototype.consumeStreamRequest = stopAwareFn("consumeStreamRequest", function (request) {
    ...
    slices.forEach(function (bs, idx) {
        ...
        var done = request.bytes === 0 && idx === lastIdx;
        request.callback(bs, done);
    });
    if (request.bytes > 0) {
        if (verbose) {
