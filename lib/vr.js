...
LittleEndianMixin.prototype.setUInt32 = function (buffer, idx, v) {
    if (idx === undefined) {
        idx = 0;
    }
    return buffer.writeUInt32LE(v, idx * 4, true);
};

LittleEndianMixin.prototype.setUInt16 = function (buffer, idx, v) {
    if (idx === undefined) {
        idx = 0;
    }
    return buffer.writeUInt16LE(v, idx * 2, true);
};

LittleEndianMixin.prototype.setFloat32 = function (buffer, idx, v) {
    if (idx === undefined) {
        idx = 0;
    }
    return buffer.writeFloatLE(v, idx * 4, true);
};
...
BigEndianMixin.prototype.setUInt32 = function (buffer, idx, v) {
    return buffer.writeUInt32BE(v, idx * 4, true);
};

BigEndianMixin.prototype.setUInt16 = function (buffer, idx, v) {
    return buffer.writeUInt16BE(v, idx * 2, true);
};

BigEndianMixin.prototype.setFloat32 = function (buffer, idx, v) {
    return buffer.writeFloatBE(v, idx * 4, true);
};
...
DICOM.FixedLength.prototype.encodeValues = function (val) {
    var count = val.length,
        buff = new Buffer(this.singleValueLength * count),
        i;
    for (i = 0; i < count; i += 1) {
        this.encodeValue(buff, i, val[i]);
    }
    return buff;
};
...
DICOM.Stringish.prototype.paddingChar = ' ';
...
DICOM.Stringish.prototype.encodeValues = function (val) {
    /// XXX: String encodings other than binary
    var i, v, len = val.length, acc = [];
    for (i = 0; i < len; i += 1) {
        v = val[i];
        assert.equal(typeof v, 'string', 'Argument must be array of strings');
        acc[i] = v;
    }
    v = acc.join('\\');
    if (v.length % 2) {
...
DICOM.UL.prototype.encodeValue = function (raw, idx, val) {
    assert.equal(typeof val, 'number', 'argument must be number');
    return this.setUInt32(raw, idx, val);
};
...
DICOM.US.prototype.encodeValue = function (raw, idx, val) {
    assert.equal(typeof val, 'number', 'argument must be number');
    return this.setUInt16(raw, idx, val);
};
/**
 * OB:  bytes
 */
...
DICOM.OB.prototype.encodeValues = function (v) {
    assert.ok(util.isArray(v), 'argument must be array');
    assert.equal(v.length, 1, 'argument must have length 1');
    return v[0];
};
...
DICOM.OF.prototype.encodeValue = function (raw, idx, v) {
    assert.equal(typeof v, 'number', 'argument must be number');
    return this.setFloat32(this.rawValue, idx, v);
};
...
DICOM.NoVR.prototype.getValues = function (buff) {
    return [buff];
};
...
DICOM.ST.prototype.encodeValues = function (v) {
    assert.ok(util.isArray(v), 'argument must be array');
    assert.equal(v.length, 1, 'argument must have length 1');
    var str = v[0];
    assert.equal(typeof str, 'string', 'argument must be string');
    if (str.length % 2) {
        str = str + ' ';
    }
    // XXX: encodings other than binary
...
DICOM.UI.prototype.paddingChar = '\u0000';
...
/**
 * AE: Application Entity, 16 bytes
 ...
 */
DICOM.AE = function (obj) {
    DICOM.VR.call(this, obj);
};
util.inherits(DICOM.AE, DICOM.CS);
DICOM.AE.prototype.vr = 'AE';
