 ...
 *
 */
var ObjectEmitter = function (ts, obj) {
    EventEmitter.call(this);
    if (obj === undefined) {
        obj = ts;
        ts = uids.ts.ExplicitVRLittleEndian;
    }
    if (typeof ts === 'string') {
        ts = uids.ts[ts];
    }
...
ObjectEmitter.prototype.emitEvents = function () {
    ...
    var self = this,
        recurse = function (obj) {
            var k, v, i, len, tag, elem, encoded, objTagsLen, 
            normObj = {}, 
            objTags = [];
            for (k in obj) {
            ...
            objTags.sort();
            objTagsLen = objTags.length;
            for (k = 0; k < objTagsLen; k += 1) {
                tag = objTags[k];
                v = normObj[tag];
                tag = tags.tag(tag);
                vtrace("ObjectEmitter sorted tag:", tag.tag);
                elem = new self.endianess[tag.vr]();
                elem.tag = tag.tag;
                ...
                if (elem.vr === 'SQ') {
                    ...
                    len = v.length;
                    encoded = elem.encodeValues(v);
                    vtrace("ObjectEmitter emitting data", encoded);
                    self.emit('data', encoded);
                } else {
                    encoded = elem.encodeValues([v]);
                    vtrace("ObjectEmitter emitting data", encoded);
                    self.emit('data', encoded);
        ...
        emitItem = function (item) {
            var elem = new self.endianess.NoVR();
            elem.tag = tags.Item;
    ...
    recurse(self.obj);
    vtrace("ObjectEmitter: emitting end");
    self.emit('end');
};
