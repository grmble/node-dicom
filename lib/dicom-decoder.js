...
var verboseDebugVrStrDataElement = function (msg, vrStr, dataElement) {
    if (verbose && log.isDebugEnabled()) {
        log.debug(msg + ":" + vrStr, dataElement.parsedObj());
    }
};
...
var groupTypeSetter = function (type) {
    return function (theGroup) {
        theGroup.type = type;
    };
};
        ...
        function vrHandler() {
            ...
            self.request(bytes, function (buffer) {
                ...
                if (self.group() && self.group().encapsulated) {
                    if (encapsulatedTags[tag]) {
                        self.request(length, function (rawValue) {
                            if (log.isDebugEnabled()) {
                                log.debug("decodeDataElement Encapsulated:", dataElement);
                            }
                        ...
                        self.exitGroup();
                        verboseDebugVrStrDataElement("end of encapcs group", vrStr, dataElement);
                        callback(dataElement);
                    } else {
                        self.onError(new Error("only item and delimitation items allowed"));
                    }
                } else if (vrStr === 'SQ' || noValueTags[tag]) {
                    mustEnter = (vrStr === 'SQ' || tag === tags.Item.tag);
                    // vrStr is either SQ or NoValue or Delimitation/Item tags
                    if (length === unlimitedLength &&  mustEnter) {
                        verboseDebugVrStrDataElement("Unlimited", vrStr, dataElement);
                        self.enterGroup(groupTypeSetter(vrStr), function (theGroup) {
                            verboseDebugVrStrDataElement("End of Unlimited", vrStr, dataElement);
                        });
                    } else if (mustEnter) {
                        verboseDebugVrStrDataElement("Limited", vrStr, dataElement);
                        self.enterGroup(length, groupTypeSetter(vrStr), function () {
                            verboseDebugVrStrDataElement("End of Limited", vrStr, dataElement);
                        });
                    } else {
                        verboseDebugVrStrDataElement("Explicit Group end", vrStr, dataElement);
                        (function () {
                            /*jslint white: true */
                            var theGroup = self.group();
                            if (theGroup && 
                                ((theGroup.type === 'SQ' && 
                                  dataElement.tag === tags.SequenceDelimitationItem.tag) ||
                                 itemTags[tag])) {
                                self.exitGroup();
                            } else {
                    ...
                    verboseDebugVrStrDataElement("Encapsulated", vrStr, dataElement);
                    self.enterGroup(function () {
                        verboseDebugVrStrDataElement("End of Encapsulated", vrStr, dataElement);
                    }).encapsulated = true;
                    callback(dataElement);
                } else {
                    self.request(length, function (rawValue) {
 ...
 *
 */
DicomDecoder.prototype.decode = function () {
    var self = this,
...
if (require.main === module) {
    ...
    decoder.decodeMetaInfo(function () {
        ...
        log.info("DecodeMetaInfo done in", t2 - t1);
        decoder.decode(function () {
            var t3 = new Date().getTime();
            log.info("Total decode done in", t3 - t1);
        });
    });
}
